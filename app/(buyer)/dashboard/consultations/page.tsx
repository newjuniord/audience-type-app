...
export default function ConsultationsHistoryPage() {
    ...
    useEffect(() => {
        ...
        const fetchBookings = async () => {
            try {
                setBookings(await getBookingApplicationsByUser(user.uid));
            } catch (err) {
                ...
                <div className="relative">
                    ...
                    <ol className="space-y-5">
                        {filtered.map((booking, idx) => {
                            // On affiche la date du rendez-vous ; `createdAt` ne sert que
                            // pour les anciennes demandes enregistrées sans créneau.
                            const date = toDate(booking.startUtc)
                                ?? (booking.createdAt ? new Date(booking.createdAt as any) : undefined);
                            const day = date ? date.getDate() : "—";
                            const month = date ? MONTHS_HT[date.getMonth()] : "";
                            const year = date ? date.getFullYear() : "";
                            const dayOfWeek = date ? DAYS_HT[date.getDay()] : "";
                            const status = getStatusConfig(booking.status);
                            // Les champs structurés existent depuis la connexion à Firestore ;
                            // le repli sur `message` couvre l'historique.
                            const fallback = parseBookingMessage(booking.message);
                            const parsed = {
                                kategori: booking.category || fallback.kategori,
                                sujet: booking.subject || fallback.sujet,
                                creneau: booking.customerTime
                                    ? `${booking.customerTime}${booking.customerTimezone ? ` · ${booking.customerTimezone.split("/").pop()?.replace(/_/g, " ")}` : ""}`
                                    : fallback.creneau,
                            };
