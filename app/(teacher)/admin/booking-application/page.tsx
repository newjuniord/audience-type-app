...
/** Utilisé uniquement pour les demandes anciennes qui n'ont pas enregistré leur fuseau. */
const FALLBACK_COACH_TZ = "Asia/Seoul";
...
export default function BookingsManagementPage() {
    ...
    // Horloges : une par fuseau réellement présent dans les demandes, plus celle de l'admin.
    const [clocks, setClocks] = useState<Record<string, string>>({});
    const adminTz = detectBrowserTimezone();
    // Filtre par coach : l'agenda peut contenir plusieurs formateurs.
    ...
    // Helpers
    // Date du rendez-vous dans l'agenda du coach (c'est son planning qu'on affiche).
    ...
    /**
     * Instant absolu du rendez-vous.
     *
     * `startUtc` est la source de vérité. Pour les demandes créées avant sa mise en place,
     * on reconstruit l'instant depuis la date/heure du coach et SON fuseau — jamais avec
     * un décalage figé, qui se trompe d'une heure dès que l'heure d'été s'applique.
     */
    const getBookingInstant = (app: ExtendedApplication): Date | null => {
        const stored = toDate(app.startUtc);
        if (stored) return stored;
        if (app.bookingDate && app.bookingTime) {
            return zonedTimeToUtc(app.bookingDate, app.bookingTime, app.serviceTimezone || FALLBACK_COACH_TZ);
        }
        return null;
    };

    /** Heure du rendez-vous dans un fuseau donné, ou "—" si l'instant est inconnu. */
    const timeIn = (app: ExtendedApplication, timeZone: string) => {
        const instant = getBookingInstant(app);
        if (!instant) return "—";
        return formatInTimeZone(instant, timeZone, { hour: '2-digit', minute: '2-digit', hour12: true }, 'fr-FR');
    };

    /** Fuseau du coach pour cette demande. */
    const coachTzOf = (app: ExtendedApplication) => app.serviceTimezone || FALLBACK_COACH_TZ;
    /** Fuseau du client, à défaut celui du coach. */
    const customerTzOf = (app: ExtendedApplication) => app.customerTimezone || coachTzOf(app);
    ...
    // Pays du client : la demande l'enregistre désormais ; sinon on le déduit du numéro
    // via le même catalogue que le formulaire de réservation.
    const getCountryInfo = (app: ExtendedApplication) => {
        const fromRecord = app.customerCountry
            ? COUNTRIES.find((c) => c.code === app.customerCountry)
            : undefined;
        const country = fromRecord || detectCountryFromPhone(app.userPhone || "");
        return country
            ? { name: country.name, flag: country.flag }
            : { name: "Non spécifié", flag: "🌐" };
    };
    ...
    // Fuseaux à afficher : ceux des coachs présents dans les demandes + celui de l'admin.
    const clockZones = useMemo(() => {
        const zones = new Set<string>([adminTz]);
        for (const app of applications) zones.add(app.serviceTimezone || FALLBACK_COACH_TZ);
        return [...zones];
    }, [applications, adminTz]);

    useEffect(() => {
        const update = () => {
            const now = new Date();
            const next: Record<string, string> = {};
            for (const zone of clockZones) {
                next[zone] = formatInTimeZone(now, zone, { hour: '2-digit', minute: '2-digit', hour12: true }, 'fr-FR');
            }
            setClocks(next);
        };
    ...
    const filteredApps = useMemo(() => {
        return applications.filter(app => {
            ...
            const matchesCoach = coachFilter === 'all' || (app.coachId || '__unassigned__') === coachFilter;
            ...
            return matchesSearch && matchesStatus && matchesPayment && matchesTime && matchesCoach;
        });
    }, [applications, searchTerm, statusFilter, paymentFilter, timeFilter, coachFilter]);

    /** Coachs présents dans les demandes, pour proposer un filtre. */
    const coachOptions = useMemo(() => {
        const map = new Map<string, { id: string; name: string; count: number }>();
        for (const app of applications) {
            const id = app.coachId || '__unassigned__';
            const existing = map.get(id);
            if (existing) existing.count += 1;
            else map.set(id, { id, name: app.coachName || 'Non assigné', count: 1 });
        }
        ...
        <main className="max-w-6xl mx-auto animate-in fade-in duration-700 relative pb-20 px-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <div className="flex flex-wrap items-center gap-3 bg-white dark:bg-white/5 border border-black/5 dark:border-white/10 px-5 py-2.5 rounded-full text-xs font-bold shadow-sm">
                        <span className="text-black/40 dark:text-white/40">Il est :</span>
                        {clockZones.map((zone, i) => (
                            <div key={zone} className="flex items-center gap-1.5">
                                {i > 0 && <div className="border-l border-black/10 dark:border-white/10 h-3 mr-1.5" />}
                                <span className="text-black/40 dark:text-white/40">
                                    {zone === adminTz ? "Vous" : getTimezoneLabel(zone)}
                                </span>
                                <span className="font-mono text-primary dark:text-white">{clocks[zone] || "--:--"}</span>
                            </div>
                        ))}
                    </div>
            ...
            {/* Filtre par coach — n'apparaît que s'il y a plusieurs formateurs. */}
            {coachOptions.length > 1 && (
                <div className="flex flex-wrap gap-2 mb-8">
                    ...
                    >
                        Tous les coachs ({applications.length})
                    </button>
                                ...
                                >
                                    ...
                                    <div className="space-y-1.5 mt-2">
                                        {cell.bookings.slice(0, 3).map((b) => (
                                            ...
                                            >
                                                {timeIn(b, coachTzOf(b))} {b.userName.split(' ')[0]}
                                            </div>
                                        ))}
                ...
                <div className="space-y-6">
                    {loading ? (
                        ...
                        filteredApps.map((app) => {
                            ...
                            const coachTz = coachTzOf(app);
                            const customerTz = customerTzOf(app);
                            const platform = getPlatformInfo(app.message);
                            const country = getCountryInfo(app);
                                        ...
                                        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                            ...
                                            <div className="bg-black/[0.02] dark:bg-white/[0.03] rounded-xl p-3 flex flex-col gap-0.5 border border-black/5 dark:border-white/5">
                                                ...
                                                <span className="font-mono text-sm font-bold text-primary dark:text-white leading-snug">
                                                    {timeIn(app, coachTz)}<br/>
                                                    <span className="text-black/50 dark:text-white/50">{country.flag} {timeIn(app, customerTz)}</span>
                                                </span>
                                            </div>
                                        </div>
                            ...
                            {dayBookings.map((app) => {
                                ...
                                return (
                                    <div key={app.id} className="bg-white dark:bg-black/20 border border-black/5 dark:border-white/10 p-6 rounded-3xl flex flex-col justify-between gap-6 hover:border-black/20 dark:hover:border-white/20 transition-all shadow-sm relative overflow-hidden">
                                        ...
                                        {/* Même instant, trois points de vue : le coach sait quand se
                                            connecter, le client aussi, et l'admin voit l'heure de son écran. */}
                                        <div className="grid grid-cols-3 gap-3">
                                            <div className="bg-primary/5 border border-primary/20 rounded-2xl p-4">
                                                <div className="text-[9px] font-black uppercase tracking-widest text-primary mb-1">
                                                    Coach {app.coachName ? `· ${app.coachName.split(' ')[0]}` : ''}
                                                </div>
                                                <div className="font-mono text-base font-bold text-primary dark:text-white">{timeIn(app, coachTz)}</div>
                                                <div className="text-[8px] text-black/40 dark:text-white/40 mt-1">{getTimezoneLabel(coachTz)} · {formatUtcOffset(coachTz)}</div>
                                            </div>
                                            <div className="bg-black/[0.02] dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl p-4">
                                                <div className="text-[9px] font-black uppercase tracking-widest text-black/40 dark:text-white/40 mb-1">
                                                    {country.flag} Client
                                                </div>
                                                <div className="font-mono text-base font-bold text-primary dark:text-white">{timeIn(app, customerTz)}</div>
                                                <div className="text-[8px] text-black/40 dark:text-white/40 mt-1">{getTimezoneLabel(customerTz)} · {formatUtcOffset(customerTz)}</div>
                                            </div>
                                            <div className="bg-black/[0.02] dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-2xl p-4">
                                                <div className="text-[9px] font-black uppercase tracking-widest text-black/40 dark:text-white/40 mb-1">Votre écran</div>
                                                <div className="font-mono text-base font-bold text-primary dark:text-white">{timeIn(app, adminTz)}</div>
                                                <div className="text-[8px] text-black/40 dark:text-white/40 mt-1">
                                                    {app.durationMinutes ? `${app.durationMinutes} min` : 'Durée non précisée'}
                                                </div>
                                            </div>
                                        </div>
