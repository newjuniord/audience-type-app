...
function stripHtmlAndTruncate(html: string | undefined | null, length: number = 100) {
    if (!html) return "";
...
export default function FeaturedProducts({
    ...
    useEffect(() => {
        ...
        async function syncOwnedStatus() {
            ...
            try {
                const enrollments = await getEnrollmentsByUser((user as any).uid || (user as any).id as string);
        ...
        syncOwnedStatus();
    }, [user?.uid, initialProducts?.length]);
    ...
    const handleProductClick = (product: Product) => {
        ...
        const detailUrl = product.type === "Course" ? `/courses/${product.id}` : `/ebooks/${product.id}`;
        router.push(detailUrl);
    };

    const mainProducts = products.filter(p => p.type !== "Service");
    const consultationProduct = products.find(p => p.type === "Service" && !!p.id && !!p.title);
        ...
        <section className={`w-full max-w-[1200px] px-6 pb-20 pt-8 ${showBorder ? 'border-t border-white/5' : ''}`}>
            ...
            {mainProducts.length > 0 ? (
                        ...
                        {visibleProducts.map((product, index) => {
                            ...
                            return (
                                <div key={index} className="group flex flex-col bg-white/[0.03] overflow-hidden border border-white/10 hover:border-primary/50 hover:bg-white/[0.06] transition-all duration-300 rounded-2xl animate-in fade-in slide-in-from-bottom-4 duration-500">
                                    <Link href={detailUrl} className="aspect-[4/3] overflow-hidden relative block">
                                    ...
                                    <div className="p-5 flex flex-col flex-grow">
                                        <div className="flex justify-between items-start mb-2 gap-3">
                                            ...
                                            <span className="text-primary font-bold text-sm shrink-0">
                                                <span>{product.price}{product.priceHTG ? ` / ${product.priceHTG.toLocaleString()} HTG` : ''}</span>
                                            </span>
                                        </div>
                                        <p className="text-xs text-white/50 mb-6 line-clamp-2 leading-relaxed"><span>{stripHtmlAndTruncate(product.description)}</span></p>
                                        <div className="mt-auto">
                                            <BubbleButton
                                                onClick={() => handleProductClick(product)}
                                            >
                                                <span>
                                                    {product.isOwned ? "Antre nan kontni" : "Wè Detay & Aksè"}
                                                </span>
                                            </BubbleButton>
                                        </div>
                                    </div>
                ...
                <div className="flex flex-col items-center justify-center py-20 bg-white/[0.02] border border-white/5 rounded-2xl">
                    ...
                    <p className="text-white/40 uppercase font-bold tracking-widest text-sm text-center">
                        Nou ap prepare pwodui yo pou ou. Tounen byento !
                    </p>
                </div>
            )}

            {consultationProduct && consultationProduct.id && mainProducts.length > 0 && (
                <div className="w-full h-px bg-primary/10 dark:bg-white/10 my-20"></div>
            )}

            {consultationProduct && consultationProduct.id && (
                            ...
                            <div>
                                <div className="flex flex-wrap items-baseline justify-between gap-4 mb-4">
                                    ...
                                    <span className="text-2xl font-extrabold text-primary">
                                        <span>{consultationProduct.price}{consultationProduct.priceHTG ? ` / ${consultationProduct.priceHTG.toLocaleString()} HTG` : ''}</span>
                                    </span>
                                </div>
                                <p className="text-base text-white/60 mb-8 max-w-2xl leading-relaxed">
                                    <span dangerouslySetInnerHTML={{ __html: consultationProduct.description }} />
                                </p>
