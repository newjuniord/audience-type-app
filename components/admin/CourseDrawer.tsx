...
export default function CourseDrawer({ isOpen, onClose, initialData, onSave }: CourseDrawerProps) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            showToast(initialData ? "Cours modifié avec succès !" : "Cours créé avec succès !", "success");
            onClose();
        } catch (error) {
    ...
    return (
        <div className={`fixed inset-0 z-[100] transition-all duration-700 overflow-hidden ${isOpen ? 'visible' : 'invisible delay-700'}`}>
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto px-8 py-8 space-y-10 custom-scrollbar">
                    ...
                    <section className={`transition-all duration-700 delay-200 ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                        <label className="block text-[10px] font-black uppercase text-black/40 dark:text-white/40 tracking-widest mb-4">Miniature du cours (URL ou Fichier)</label>
                        <div className="relative group">
                            ...
                            <div 
                                ...
                                className="w-full aspect-video rounded-3xl border-2 border-dashed border-black/10 dark:border-white/10 bg-black/[0.02] dark:bg-white/[0.02] flex flex-col items-center justify-center cursor-pointer hover:border-primary/30 dark:hover:border-white/30 transition-all overflow-hidden shadow-inner"
                            >
                                {isUploading ? (
                                    <div className="flex flex-col items-center gap-2">
                                        <span className="material-symbols-outlined text-3xl animate-spin text-primary">refresh</span>
                                        <span className="text-xs font-bold text-black/50 dark:text-white/50">Upload en cours...</span>
                                    </div>
                                ) : thumbnail ? (
                                    ...
                                    <div className="flex flex-col items-center gap-2 group-hover:scale-110 transition-transform duration-500">
                                        <span className="material-symbols-outlined text-3xl text-black/20 dark:text-white/20">upload</span>
                                        <span className="text-xs font-bold text-black/40 dark:text-white/40">Cliquez pour uploader</span>
                                        <input
                                            type="text"
                                            placeholder="Ou collez l'URL de l'image ici"
                                            className="bg-transparent border-none text-center text-xs focus:ring-0 placeholder:text-black/20 mt-2"
                                            value={thumbnail}
                    ...
                    <section className={`space-y-6 transition-all duration-700 delay-300 ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                />
                                <p className="text-[10px] text-black/30 dark:text-white/30 mt-1 ml-2">ID de la variante (Lemon Squeezy)</p>
                            </div>
                            <div>
            ...
            <ConfirmModal
                ...
                message="Êtes-vous sûr de vouloir publier ce cours ? Il sera visible par tous les étudiants."
                confirmText="Publier"
                cancelText="Annuler"
            />
        </div>
