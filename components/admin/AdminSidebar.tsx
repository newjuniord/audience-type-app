...
const MENU_SECTIONS = [
    ...
    },
    {
        ...
        items: [
            ...
            { label: "Sondaj 📊", icon: "poll", href: "/admin/surveys" },
...
export default function AdminSidebar({ isOpen = true, onToggle }: AdminSidebarProps) {
    ...
    useEffect(() => {
        ...
        if (!activeSectionTitle) {
            const hasAnyActive = MENU_SECTIONS.some(section => 
            ...
            <div className="p-6 border-t border-black/5 dark:border-white/10">
                <div className="flex items-center gap-3 p-2">
                    <div className="size-10 rounded-full bg-black/10 overflow-hidden relative flex items-center justify-center">
                        {(user as any)?.photoURL || (user as any)?.user_metadata?.photoURL ? (
                            <img
                                alt="Admin Profile"
                                className="w-full h-full object-cover"
                                src={(user as any)?.photoURL || (user as any)?.user_metadata?.photoURL}
                            />
                        ) : (
                    ...
                    <div className="overflow-hidden">
                        <p className="text-xs font-bold truncate max-w-[120px]">{userData?.displayName || (userData as any)?.name || (user as any)?.displayName || (user as any)?.user_metadata?.displayName || "Admin User"}</p>
