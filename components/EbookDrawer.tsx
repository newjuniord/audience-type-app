        ...
        <div className={`fixed inset-0 z-[100] transition-all duration-700 overflow-hidden ${isOpen ? 'visible' : 'invisible delay-700'}`}>
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto px-8 py-8 space-y-8 custom-scrollbar">
                    {/* Ebook Cover Upload (Mock for now, creates text input for URL) */}
                    <section className={`transition-all duration-700 delay-200 ${isOpen ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                        <label className="block text-[10px] font-black uppercase text-black/40 dark:text-white/40 tracking-widest mb-4">Couverture (URL Image)</label>
                        <div className="flex flex-col gap-4">
                            <input
                                className="w-full h-10 px-4 rounded-xl bg-black/[0.03] dark:bg-white/[0.03] border-none text-sm"
                                placeholder="https://..."
                                type="text"
                                value={coverImage}
                            ...
                            />
                            {coverImage && (
                                <div className="aspect-[3/4] w-32 rounded-lg overflow-hidden border border-black/10">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex items-center">
                                ...
                                <button
                                    ...
                                    disabled={!fileUrl}
                                    className="px-6 h-14 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-primary dark:text-white rounded-r-2xl font-bold text-xs uppercase tracking-wider transition-all disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                                >
                                    Tester le lien
                                </button>
                            </div>
